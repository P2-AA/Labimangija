...
public class AvlElemendiLisamine {
    ...
    public void laePuu() {
        String valitudFail = AlgorithmSisendiValija.valiSisend(AlgorithmSisendiValija.Tyyp.AVL_LISAMINE);
        if (valitudFail == null) {
            return;
        }
        juhend.setText("");
    ...
    public void lukustaPuuOlek() {
        if (!lisatud) {
            Popups.showError("Enne tipu lisamist ei saa puud lukku panna");
            return;
        }
        ...
        if (onBst && onAvl) {
            ...
            if (onBst) {
                ...
                Popups.showError("Ebakorrektne lisamine ja ei ole säilinud AVL-puu");
            } else {
                vead.add("VIGA: " + lisatavad.get(0) + " lisati ebakorrektselt, kahendotsimispuu ei säilinud");
                Popups.showError("Ebakorrektne lisamine ja puu ei ole enam Kahendotsimispuu!");
            }
            vigu++;
            visuaalnePuu = new Kahendotsimispuu();
            puudSamaks(visuaalnePuu, eelnevaSeisugaPuu.juurtipp);
